...
global.coreDevDonationAddress = global.devDonationAddress = {
    ...
    'haven' : 'hvxy4fEM7gvaCfoYwDZnZUdB4UxCec3i9a8eowCWNxnoj9pQzmiBNgoXLrU6RZtcthByBrWP6x55jYMRGUQkx5Ze5Vr28iH87h'
};
global.doDonations = {};
for(var i=0; i<config.coins.length; i++) {
...
global.config.coin = function(ctx) {
    ...
    if(ctx.hostname) { // context is Express request
        ...
        hostNames = (ctx.headers.host ?
                        ctx.headers.host :
                            url.parse(ctx.headers.origin ?
                                ctx.headers.origin :
                                    ctx.headers.referer).host).split('.');
    } else {
        log('error', 'general', 'Unknown context type');
        return 'electroneum';
    }
    ...
    } else if(!isNaN(topName)) {
        topName = config.symbols[0].toLowerCase(); // request made by IP address, use default currency.
    }
    ...
    log('warn', 'general', 'Coin %s not in the config', [topName]);
    return 'electroneum';
};