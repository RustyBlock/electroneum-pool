    ...
    async.waterfall([
        ...
        function(blocks, callback){
            async.filter(blocks, function(block, mapCback){
                apiInterfaces[coin].rpcDaemon('getblockheaderbyheight', {height: block.height}, function(error, result){
                    ...
                    block.unlocked = blockHeader.depth >= config.blockUnlocker.depth[coin];
                    block.reward = blockHeader.reward - (blockHeader.reward * config.blockUnlocker.networkLevy[coin] / 100);
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                var feePercent = config.blockUnlocker.poolFee[coin] / 100;
        ...
        setTimeout(function() { runInterval(coin); }, config.blockUnlocker.interval * 1000);
    })
}
...
function adjustBlockRewardForCloudMiners(coin, block) {
    ...
    // save adjusted share total
    redisClient.set(coin + ':shares:round-' + block.height + ':hashes', totalShares);
}
