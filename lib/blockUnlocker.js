    ...
    async.waterfall([
            ...
            blocks.forEach(function(block){
                ...
                if (block.workerShares) {
                    ...
                    Object.keys(workerShares).forEach(function (worker) {
                        ...
                        orphanCommands.push(['zincrby', config.coin + ':shares:round-0', workerShares[worker], worker]);
                    });
                }
            });
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                if (block.workerShares) {
                    adjustBlockRewardForCloudMiners(block);
                    ...
                    Object.keys(block.workerShares).forEach(function (worker) {
                        ...
                        saveBlockHistory(worker, block.height.toString());
                    });
                }
            });
...
function saveBlockHistory(worker, blockHeight) {
    var key = config.coin + ':workers:' + worker;
    redisClient.hget(key, 'blocks', function(error, result) {
        ...
        redisClient.hset(key, 'blocks', value, function(error, result) {
            if(error) {
...
function adjustBlockRewardForCloudMiners(block) {
    var totalShares = parseInt(block.shares);
    Object.keys(block.workerShares).forEach(function (worker) {
        if(worker.substr(worker.length - 6) !== ':cloud') {
            return;
        ...
        totalShares -= penalty;
        redisClient.zincrby(config.coin + ':shares:round-' + block.height, penalty, miner + ':penalty', function(error, result) {
            if(error) {
