...
exports.isValidAddress = function(addr, coin, addressInfo){

    var addrBuf = new Buffer(addr),
        minerPref = cnUtil.address_decode(addrBuf),
        poolPrefs = addressBase58Prefix;
    addressInfo = (typeof addressInfo === 'undefined') ? {} : addressInfo;

    if(isNaN(minerPref)) {
        minerPref = cnUtil.address_decode_integrated(addrBuf);
        addressInfo.isIntegrated = true;
        poolPrefs = addressBase58PrefixInt;
    } else {
        addressInfo.isIntegrated = false;
    }
