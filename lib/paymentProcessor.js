...
function runInterval(coin){
    async.waterfall([
        ...
        function(balances, callback){
            ...
            var transferCommands = [], transferCommandsPayId = [], txs;
            var addresses = 0;
            var commandIndex = 0, commandPayIdIndex = 0, idx;
            var addressInfo = {};
            ...
            for (var worker in payments) {
                ...
                txs = transferCommands;
                idx = commandIndex;

                if(addressAndPaymentId.length > 1 || addressInfo.isIntegrated) {
                    if(addressAndPaymentId.length > 1) { // set payment ID if specified
                        blank.rpc.payment_id = addressAndPaymentId[1];
                    }
                    transferCommandsPayId.push(blank);
                    txs = transferCommandsPayId;
                    idx = commandPayIdIndex;
                    commandPayIdIndex++;
                } else {
                    addresses++;
                }

                txs[idx].rpc.destinations.push({amount: amount, address: addressAndPaymentId[0]});
                txs[idx].redis.push(['hincrby', coin + ':workers:' + worker, 'balance', -amount]);
                txs[idx].redis.push(['hincrby', coin + ':workers:' + worker, 'paid', amount]);
            ...
            async.filter(transferCommands.concat(transferCommandsPayId), function(transferCmd, cback){
