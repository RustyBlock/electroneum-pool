...
function processNotifications() {
    ...
    function scan() {
        redisClient.scan(cursor, 'MATCH', config.coin + ':auth:users:*', 'COUNT', '200', function (error, reply) {
            if(error){
    ...
    wallets.forEach(function(wallet) {
        ...
        redisClient.sort(baseKey + ':active-count', 'LIMIT 0 2', 'DESC', function(error, result) {
            ...
            redisClient.multi(redisCommands).exec(function(error, results) {
                ...
                avgRate = (results[0] + results[2]) / (results[1] + results[2]);
                ...
                if(nf.hiEnabled && avgRate >= nf.hiRate) {
                    notifyUser(userKey, avgRatem, wallet, false);
                }
            });
        });
    });
