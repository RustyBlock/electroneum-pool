...
var rateNotification = { // base notification object
    miderId: 'wallet-address',
    hashrate: 1234,
    limitRate: 1234,
    referenceUrl: config.www.host,
    bodyTemplate: 'Hello,<br/><br/><i>this is notification from the cryptocurrency mining pool where your email was subscribed to receive updates.</i><br/>' + 
    'Miner on %s H/s.<br/>' +
    'You can come back to the pool to <a href="%s?miner=%s">review the latest miner statistics.</a> or change notification settings in <a href="%s#profile">your user profile</a>.<br/><br/>' +
    '--<br/><b>RustyLock Team</b></br><a href="mailto:' + process.env.process.env.emailAddressFrom + '">' + process.env.emailAddressFrom + '</a>',
    format: function() {
        return util.format(this.body, this.miderId, this.hashrate, this.limitRate); 
    }
}, rateTooLow = Object.assign({ // notification about low rate
    subject: 'Hahsrate is too low',
    body: util.format(this.bodyTemplate, '%%s has dropped hashrate to %s H/s which is <b style="color:red;">lower</b> than your specified minimum rate of %$%s')
}, rateNotification),
rateTooHigh = Object.assign({ // notification about high rate
    subject: 'Hahsrate is too high',
    body: util.format(this.bodyTemplate, '%%s has exceeded hashrate to %s H/s which is <b style="color:red;">hgher</b> than your specified maximum rate of %%s')
}, rateNotification), logSystem = 'pool';
