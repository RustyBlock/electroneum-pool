...
exports.saveMinerActivity = function(miner, shareDiff, dateNow) {
    
    var dateNowSeconds = fixedMinuteIntervalStart(new Date(dateNow), 5); // aggregate by fixed 5 minute intervals

    redisClient.zincrby(config.coin + ':auth:wallets:' + miner.login + ':active-stats', shareDiff, dateNowSeconds, function(error, replies) {
        if(error) {
    ...
    async.each(wallets, function(wallet, callback) {
        ...
        redisClient.multi(redisCommands).exec(function(error, results) {
            if(error){
                ...
                log('info', logSystem, 'NF state for %s within %s-%s: %s-%s, %s, %s, %s', 
                    [wallet, beforeLastInterval, lastInterval, results[0], results[1], results[2], results[3], results[4]]);
            }
            ...
            async.parallel([
                function(callback) {
                    ...
                    if(avgRate <= nfState.loRate) {
                        ...
                        if(avgRate === 0 && alreadySentZero !== 'true') {
                            notifyUser(userKey, avgRate, wallet, nfState, email, rateZero, function(error) {
                                ...
                                redisClient.multi([
                                    ['hset', userKey + ':hashNf', 'sentLow', 'true'],
                                    ['hset', userKey + ':hashNf', 'sentZero', 'true']
                        ...
                        if(alreadySentLow !== 'true' && alreadySentZero !== 'true') {
                    ...
                    } else { // clear "already sent" flags if current speed is higher than limit
                        redisClient.multi([
                            ['hdel', userKey + ':hashNf', 'sentLow'],
                            ['hdel', userKey + ':hashNf', 'sentZero']
                            ...
                            if(error) {
                                ...
                                if(result) {
                                    log('info', logSystem, 'Cleared low rate sent flags on %s', [wallet]);
                                }
                                callback();
                            }
                        });
                    ...
                    if(avgRate >= nfState.hiRate) {
                        ...
                        redisClient.hdel(userKey + ':hashNf', 'sentHigh', function(error, result) {
                            if(error) {
                                ...
                                if(result) {
                                    log('info', logSystem, 'High rate sent flag cleared for wallet %s', [wallet]);
                                }
                                callback();
                            }
                        });
