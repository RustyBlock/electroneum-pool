...
function processNotify(userKey, email, wallets, nf) {
    ...
    wallets.forEach(function(wallet) {
        ...
        redisClient.sort(baseKey + ':active-count', 'LIMIT', '1', '2', 'DESC', function(error, result) {
        ...
        redisClient.scan(cursor, 'MATCH', config.coin + ':auth:wallets:*', 'COUNT', '300', function (error, reply) {
            ...
            if(reply[1].length > 0){
                ...
                reply[1].forEach(function(key) {
                    ...
                    var windowTime = ((Date.now() / 1000) - 259200) | 0,
                        baseKey = config.coin + ':auth:wallets:' + key; // 3 days
                    redisClient.sort(baseKey + ':active-stats', 'LIMIT', '0', '1000', function(error, vals){
                        if(error) {
                            log('error', logSystem, 'Failed to get wallet\'s expired stats: %s', [error.toString()]);
                            return;
                        }
                        log('info', logSystem, 'Sorted list, top %s', vals[vals.length - 1]);
                        if(!vals || vals.length === 0 || vals[vals.length - 1] >= windowTime) {
                            return;
                        }
                        redisClient.zrem(baseKey + ':active-stats', vals);
                        redisClient.zrem(baseKey + ':active-count', vals);
                    });
                });
            }
