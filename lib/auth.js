 ...
 * @param {string} coin name of the coin for multi-currency support
 ...
 * @return {*} none
 */
global.getWallet = function(coin, walletAddress, success, fail) {
    redisClient.lrange(coin + ":auth:wallets:" + walletAddress + ":users", 0, -1, function(err, result){
        if(err) {
            fail(err);
            return;
        }
