...
function handleMinerStats(urlParts, request, response) {
    ...
    var redisCommands = [
        ['hgetall', config.coin(request) + ':workers:' + address]
    ];

    redisCommands.push(['hget', config.coin(request) + ':shares:roundCurrent', address]);
    redisCommands.push(['zrevrange', config.coin(request) + ':payments:' + address, 0, config.api.payments - 1, 'WITHSCORES']);
    redisCommands.push(['zrevrange', config.coin(request) + ':auth:wallets:' + address + ':blocks', 0, config.api.blocks - 1, 'WITHSCORES']);
    ...
    redisClient.multi(redisCommands).exec(function (error, replies) {
        ...
        if (urlParts.query.history === 'true') {
            ...
            getHashrateHistory(config.coin(request), interval ? parseInt(interval) : 24, address,
                completeResponse,
...
for(var i = 0; i<config.coins.length; i++) {
    (function() {
        var port = config.api.port[config.coins[i]];
        server.listen(port, function () {
            log('info', logSystem, 'API started & listening on port %d', [port]);
        });
    })();
