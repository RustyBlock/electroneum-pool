...
var globalStats = require('./global-stats');
...
function collectStats(){
    ...
    async.parallel({
        ...
        if (error){
            ...
            setTimeout(function() { globalStats.saveGlobalStats(results); }, 100);
        }
...
function broadcastLiveStats(){
    ...
    for (var uid in liveConnections){
        ...
        if(res.historyFrom && res.historyTo) {
            globalStats.getHistory(res.historyFrom, res.historyTo, function(result) {
    ...
    redisClient.zrevrangebyscore(
        ...
        function(err, result) {
            ...
            redisClient.multi(redisCommands).exec(function(error, rank) {
                ...
                fullStats = globalStats.currentStatsJson;
