...
function broadcastLiveStats(){
    ...
    redisClient.multi(redisCommands).exec(function(error, replies){
        ...
        for (var i = 0; i < addresses.length; i++){
            ...
            if (!stats) {
                completeResponse(res.__acceptEncoding, res.__headers, JSON.stringify({error: "not found"}), res);
                return;
            }
            ...
            function completeRequest(results) {
                if(results) {
                    ...
                    stats.historyTimestamp = (hashrateByMiner[address] || { cachedUpTo: (new Date()).getTime() / 1000 }).cachedUpTo;
                }
                completeResponse(res.__acceptEncoding, res.__headers, JSON.stringify({stats: stats, payments: replies[offset + 2]}), res);
            }
...
function completeResponse(acceptEncoding, headers, content, response)
{
...
function handleMinerStats(urlParts, request, response){
    ...
    redisClient.multi(redisCommands).exec(function(error, replies){
        if (error || !replies[0]){
            completeResponse(acceptEncoding, headers, JSON.stringify({error: 'not found'}), response);
            return;
        }
        if (urlParts.query.longpoll === 'true') {
            ...
            function completeRequest(results){
                ...
                completeResponse(acceptEncoding, headers, JSON.stringify({stats: stats, payments: replies[2]}), response);
            }
