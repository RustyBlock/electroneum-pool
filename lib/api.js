...
var stats = require('./active-stats');
...
function broadcastLiveStats(){
    ...
    for (var uid in liveConnections){
        ...
        if((res.historyFrom && res.historyTo) || 
            ...
            poolStats.getHistory(res.historyFrom, res.historyTo, res.poolHistoryFrom, res.poolHistoryTo, function(result) {
                ...
                zlib.deflateRaw(JSON.stringify(returnObj), function(error, compressedJson){
                    if(error) {
                        log('error', logSystem, 'Failed to compress the live stats: %s', [error.toString()]);
                        return;
                    }
                    res.end(compressedJson);
                });            
            });
        } else {
    ...
    switch(urlParts.pathname){
        case '/stats':
            var headers = {
                ...
                'Content-Length': globalStats.currentStatsCompressed ? globalStats.currentStatsCompressed.length : 0
            };
            if((urlParts.query.historyFrom && urlParts.query.historyTo) || 
                (urlParts.query.poolHistoryFrom && urlParts.query.poolHistoryTo)) {
                poolStats.getHistory(urlParts.query.historyFrom, urlParts.query.historyTo, urlParts.query.poolHistoryFrom, urlParts.query.poolHistoryTo, function(result) {
                    var returnObj = clone(globalStats.currentStatsJson);
                    returnObj.history = result;
                    zlib.deflateRaw(JSON.stringify(returnObj), function(error, compressedJson){
                        if(error) {
                        ...
                        headers['Content-Length'] = compressedJson.length;
                        response.writeHead("200", headers);
                        response.end(compressedJson);
                    });            
                });
            } else {
                response.writeHead("200", headers);
        ...
        case '/live_stats':
            ...
            var uid = uuidv4();
            response.historyFrom = urlParts.query.historyFrom | urlParts.query.poolHistoryFrom;
            response.historyTo = urlParts.query.historyTo | urlParts.query.poolHistoryTo;
            response.poolHistoryFrom = urlParts.query.poolHistoryFrom | urlParts.query.historyFrom;
            response.poolHistoryTo = urlParts.query.poolHistoryTo | urlParts.query.historyTo;
