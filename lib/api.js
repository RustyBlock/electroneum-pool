...
function handleMinerStats(urlParts, request, response) {
    ...
    redisClient.multi(redisCommands).exec(function (error, replies) {
        ...
        if (urlParts.query.history === 'true') {
            ...
            getHashrateHistory(interval ? parseInt(interval) : 24, address,
                completeResponse,
...
var minerHistory = {};
function getHashrateHistory(historyHours, address, success, fail) {
    var statsKey = config.coin + ':auth:wallets:' + address + ':active-stats',
