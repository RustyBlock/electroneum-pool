...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch(method){
        ...
        case 'submit':
            ...
            if (!job){
                log('warn', logSystem, "Failed to find miner's original job id: %s", [params.job_id]);
            ...
            if (!blockTemplate){
                ...
                log('warn', logSystem, 'Block expired from %j: share height: %s, current height: %s', 
                    [miner, job.height, currentBlockTemplate.height]);
                return;
            }
            var shareAccepted;
            //try {
