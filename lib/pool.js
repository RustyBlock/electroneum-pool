...
function Miner(id, login, pass, ip, startingDiff, pushMessage, minDifficultyOverride){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
    ...
    switch(method){
        case 'login':
            ...
            global.getWallet(params.login, function(result){
                ...
                miner = new Miner(minerId, params.login, params.pass, ip, portData.difficulty, pushMessage, portData.minDifficulty);
        ...
        case 'submit':
            if (!miner){
                sendReply('Unauthenticated');
                log('warn', logSystem, 'Failed to submit shares, miner is not found: %j', [params]);
                return;
            }
            miner.heartbeat();
