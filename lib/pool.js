...
setInterval(function(){
    ...
    for (var minerId in connectedMiners){
        var miner = connectedMiners[minerId];
        if (now - miner.lastBeat && !miner.socket.writable > timeout){
...
BlockTemplate.prototype = {
    nextBlob: function() {
        ...
        if(this.blobClount > 0) {
            ...
            //log('info', logSystem, '\nStart blob: %s\n      Blob: %s', [this.originalHashingBlob, this.hashingBlob]);            
        }

        if(++this.blobClount == 180) {
            jobRefresh();
        } else if(this.blobClount === 180) {
            log('warn', logSystem, 'Forced call job refresh!');
        }
...
function handleMinerMethod(method, params, socket, portData, sendReply, pushMessage) {
    ...
    if (miner && IsBannedIp(miner.login + '@' + miner.ip)) {
    ...
    switch(method){
        ...
        case 'submit':
            ...
            var shareAccepted;
            //try {
                shareAccepted = processShare(miner, job, blockTemplate, params.nonce, params.result, portData.minDifficulty);
            //} catch (error) {
            //    log('error', logSystem, 'Failed to process share from %s, job: %j\nblock template: %j, params: %j, min diff: %s\nError: %j',
            //        [miner.login, job, blockTemplate, params, portData.minDifficulty, error]);
            //    sendReply('Failed to process share');
            //    return;
            //}
    ...
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            var sendReply = function(error, result){
                if(!socket.writable) {
                    log('warn', logSystem, 'Miner socket on %s is no longer writable', [socket.remoteAddress]);
                    return;
                }
        ...
        var onConnection = function(socket){
            ...
            var pushMessage = function(method, params){
                if(!socket.writable) {
                    log('warn', logSystem, 'Failed to push %s method to miner on %s as socket is disconnected', [method, socket.remoteAddress]);
                    return;
                }
                var sendData = JSON.stringify({
                    jsonrpc: "2.0",
