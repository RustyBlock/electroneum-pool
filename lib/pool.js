...
Miner.prototype = {
    ...
    getJob: function(){
        if (this.lastBlockHeight === currentBlockTemplate.height && !this.pendingDifficulty && 
                (new Date() - this.noJobSince < 480000)) { // no new job for 5 minutes
            return {
                blob: '',
                job_id: '',
                target: ''
...
function processShare(miner, job, blockTemplate, nonce, resultHash, minDifficulty){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        if (hash.toString('hex') !== resultHash) {
            log('warn', logSystem, 'Bad hash from miner %s@%s\last valid share:%j\ncurrent share:%j', [miner.login, miner.ip, lastValidShare,
                {
                    nonce: nonce,
    ...
    switch(method){
        case 'login':
            ...
            global.getWallet(params.login, function(result){
                ...
                var minerName = params.login + '@' + socket.remoteAddress;
                var orphans = orphanedJobs[minerName];
                miner = new Miner(minerId, params.login, params.pass, socket, portData.difficulty, pushMessage, portData.minDifficulty);
                if(orphans) {
                    miner.validJobs = orphans.slice();
                }
        ...
        case 'submit':
            if (!miner){
                sendReply('Unauthenticated');
                log('warn', logSystem, 'Failed to submit shares, miner is not found: %j, ip: %s on port %j', [params, ip, portData]);
                return;
            }
            miner.heartbeat();

            var jobIndex=-1;
            var job = miner.validJobs.filter(function(job, index){
            ...
            if(jobIndex >= 0) {
                miner.validJobs = miner.validJobs.slice(jobIndex, jobIndex+1);
            }

            if (!job){
                log('warn', logSystem, "Failed to find original job id %s (in %s jobs) for %s@%s", [params.job_id, miner.validJobs.length, miner.login, miner.ip]);
            ...
            if (!blockTemplate){
                ...
                log('warn', logSystem, 'Block expired from %s@%s: share height: %s, current height: %s', 
                    [miner.login, miner.ip, job.height, currentBlockTemplate.height]);
                return;
            }
    ...
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            var sendReply = function(error, result){
                if(!socket.writable) {
                    log('warn', logSystem, 'Miner socket for %s is no longer writable', [socket.minerId]);
                    return;
                }
        ...
        var onConnection = function(socket){
            ...
            socket.on('data', function(d){
                ...
                if (err.code !== 'ECONNRESET') {
                    ...
                    log('info', logSystem, 'Miner socket reset on %s for miner %s', [ip, socket.minerId]);
                    if(socket.minerId) {
                        delete connectedMiners[socket.minerId];    
                    }
                }
            }).on('close', function(had_error){
                pushMessage = function(){};
                log('info', logSystem, 'Miner socket closed on %s for miner %s, with error: %s', [ip, socket.minerId, had_error]);
                var miner = connectedMiners[socket.minerId];
                if(miner) {
                    var minerName = miner.login + '@' + miner.ip;
                    orphanedJobs[minerName] = miner.validJobs;
                    setTimeout(function(){
                        delete orphanedJobs[minerName];
                    }, 10000);
                    delete connectedMiners[socket.id];
                }
            });
