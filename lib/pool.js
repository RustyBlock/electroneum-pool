    ...
    switch(method){
        case 'login':
            ...
            if (addressParts.length > 1) {
                log('info', logSystem, 'Address parts after "." not supported. Ignoring: %s', [addressParts.slice(1).join('.')]);
            }
            ...
            global.getWallet(params.login, function(result){
                if(typeof result === 'undefined') {
                    sendReply(params.login + 
                        ' is not registered. Please register your user and add ' + params.login + ' to your miner wallets list in the user profile section.');
                    return;
                }
