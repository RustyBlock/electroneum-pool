    ...
    switch(method){
        case 'login':
            if (!params.login){
                ...
                log('warn', logSystem[miner.coin], 'Miner is missing login, params sent: %j', [params]);
                return;
            }
            ...
            global.getWallet(portData.coin, params.login, function(result){
                ...
                log('error', logSystem[miner.coin], 'Failed to read wallet details: %s', [err.toString()]);
                sendReply('Miner login failed. Please report to the pool admin quoting your wallet address.');
            });
            break;
        case 'getjob':
            log('info', logSystem[miner.coin], 'Get job');
            if (!miner){
        ...
        case 'submit':
            if (!miner){
                sendReply('Unauthenticated');
                log('warn', logSystem[miner.coin], 'Failed to submit shares, miner not found: %j, ip: %s on port %j, share difficulty: %s', [params, ip, portData, getHashDifficulty(new Buffer(params.result, 'hex'))]);
                return;
            }
            miner.heartbeat();
            ...
            if (!job){
                log('warn', logSystem[miner.coin], "Failed to find original job id %s for %s@%s, share difficulty: %s",
            ...
            if (!noncePattern.test(params.nonce)) {
                ...
                log('warn', logSystem[miner.coin], 'Malformed nonce: ' + JSON.stringify(params) + ' from ' + minerText);
            ...
            if (job.submissions.indexOf(params.nonce) !== -1) {
                var fullAddress = miner.login + '@' + miner.ip,
                    minerText = miner ? (' ' + fullAddress) : '';
                log('warn', logSystem[miner.coin], 'Duplicate share: ' + JSON.stringify(params) + ' from ' + minerText);
                perIPStats[fullAddress] = { validShares: 0, invalidShares: 999999 };
                miner.checkBan(false);
            ...
            if (shareTrustEnabled){
                if (shareAccepted === 0){
                    ...
                    log('warn', logSystem[miner.coin], 'Share trust broken by %s@%s', [miner.login, miner.ip]);
            ...
            if (shareAccepted !== 0) {
                ...
                switch(shareAccepted) {
                    ...
                    case 4:
                        ...
                        break;
                    default:
                        log('error', logSystem[miner.coin], 'Failed to process share with unexpected status %s', [shareAccepted]);
                        return;
                }
        ...
        default:
            ...
            log('warn', logSystem[miner.coin], 'Invalid method: %s (%j) from %s', [method, params, minerText]);
            break;
    }
}
