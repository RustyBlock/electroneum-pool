...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['zincrby', config.coin + ':shares:round-0', miner.login, job.score]
    ...
    actStats.saveMinerActivity(miner, job, shareDiff, dateNowSeconds);
}
    ...
    switch(method){
        case 'login':
            ...
            if (addressParts.length > 1) {
                ...
                log('info', logSystem, 'Miner %s is attempting to use %s for fixed diffuculty', [addressParts[0], afterDot]);
