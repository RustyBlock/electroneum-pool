...
for(var i=0;i<config.coins.length;i++) {
    (function() {
        var coin = config.coins[i], algo = config.poolServer.algos[coin];
        cryptoNight[coin] = function (data) {
            return multiHashing[algo[0]](data, algo[1]);
        }
    })();
}
...
Miner.prototype = {
    ...
    getJob: function(){
        ...
        cache.set(jobsKey + newJob.id, newJob, 600);
        return {
            blob: blob,
            job_id: newJob.id,
            target: target,
...
function processShare(miner, job, blockTemplate, nonce, resultHash, minDifficulty){
    var fromCloud = !!minDifficulty, difficulty = blockTemplate.difficulty,
        template = new Buffer(blockTemplate.blockTemplateBlob, 'hex');
    ...
    if (hashDiff.ge(difficulty)){
        log('info', logSystem, 'Block candidate found at height %d by miner %s@%s',
...
function handleMinerMethod(method, params, socket, portData, sendReply, pushMessage) {
    ...
    switch(method){
        ...
        case 'submit':
            ...
            if (!job){
                log('warn', logSystem, "Failed to find original job id %s for %s@%s, share difficulty: %s",
            ...
            var blockTemplate = 
                currentBlockTemplate[miner.coin].height === job.height ? JSON.parse(job.blockTemplate) : null;
            if (!blockTemplate){
                sendReply('Block expired');
                log('warn', logSystem, 'Block expired from %s@%s: share height: %s, current height: %s, share difficulty: %s', 
                    [miner.login, miner.ip, job.height, currentBlockTemplate[miner.coin].height, getHashDifficulty(new Buffer(params.result, 'hex'))]);
                return;
            }
