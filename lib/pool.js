...
config.coins.forEach(function(coin) {
    ...
    }, config.poolServer.varDiff[coin].retargetTime * 1000);
});
...
BlockTemplate.prototype = {
    nextBlob: function() {
        ...
        if(++this.blobClount === 120) {
            var that = this;
            jobRefresh(this.coin, false, function(res){
                that.blobClount = 0;
                log('warn', logSystem[that.coin], 'Forced to call job refresh with callback result: %s', [res]);
            });
        }
...
var VarDiff = {};
function init(coin){
    log('info', logSystem[coin], 'Initing...%s', [coin]);
    ...
    var variance = config.poolServer.varDiff[coin].variancePercent / 100 * config.poolServer.varDiff[coin].targetTime;
    VarDiff[coin] = {
            variance: variance,
            bufferSize: config.poolServer.varDiff[coin].retargetTime / config.poolServer.varDiff[coin].targetTime * 4,
            tMin: config.poolServer.varDiff[coin].targetTime - variance,
            tMax: config.poolServer.varDiff[coin].targetTime + variance,
            maxJump: config.poolServer.varDiff[coin].maxJump
        };
}
...
function Miner(id, login, pass, socket, portData, pushMessage){
    ...
    this.minDifficultyOverride = portData.minDifficulty ? 
        portData.minDifficulty : config.poolServer.varDiff[portData.coin].minDiff;
...
Miner.prototype = {
    retarget: function(now){
        ...
        var options = config.poolServer.varDiff[this.coin];

        var sinceLast = now - this.lastShareTime;
        var decreaser = sinceLast > VarDiff[this.coin].tMax;
        ...
        if (avg > VarDiff[this.coin].tMax && this.difficulty > this.minDifficultyOverride){
            newDiff = options.targetTime / avg * this.difficulty;
            newDiff = newDiff > options.minDiff ? newDiff : this.minDifficultyOverride;
            direction = -1;
        }
        else if (avg < VarDiff[this.coin].tMin && this.difficulty < options.maxDiff){
            newDiff = options.targetTime / avg * this.difficulty;
    ...
    getJob: function(){
        ...
        var tpl = currentBlockTemplate[this.coin];
        if (this.lastBlockHeight === tpl.height && !this.pendingDifficulty) {
            return {
                blob: '',
                job_id: '',
                target: ''
        ...
        var blob = tpl.nextBlob();
        this.lastBlockHeight = tpl.height;
        var target = this.getTargetHex();

        var newJob = {
            id: uuidv4(),
            blockTemplate: JSON.stringify(tpl),
            height: tpl.height,
        ...
        process.send({type: 'poolCache', key: jobsKey + newJob.id, data: newJob, ttl: 1800});
        return {
            blob: blob,
            job_id: newJob.id,
            target: target,
...
function processShare(miner, job, blockTemplate, nonce, resultHash, minDifficulty){
    var fromCloud = !!minDifficulty, difficulty = blockTemplate.difficulty,
        ...
        shareBuffer;
    try {
        shareBuffer = cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'));
    } catch(err) {
        log('error', logSystem[miner.coin], 'Failed to parse block template for %s@%s: %s\n%s\nNonce:%s\nShare difficulty: %s',
            [miner.login, miner.ip, err.toString(), blockTemplate.blockTemplateBlob, nonce, getHashDifficulty(new Buffer(resultHash, 'hex'))]);
        return 1;
    }
    ...
    if (hash.toString('hex').toLowerCase() !== resultHash.toLowerCase()) {
        log('warn', logSystem[miner.coin], 'Bad hash (%s) from miner %s@%s, share difficulty: %s',
            [resultHash, miner.login, miner.ip, getHashDifficulty(new Buffer(resultHash, 'hex'))]);
        return 1;
    }
    ...
    if(jobExpired) {
        log('warn', logSystem[miner.coin], 'Block expired from %s@%s: share height: %s, current height: %s, share difficulty: %s',
            [miner.login, miner.ip, job.height, blockTemplate.height, hashDiff.toString()]);
    }

    if (hashDiff.ge(difficulty) && !jobExpired){
        log('info', logSystem[miner.coin], 'Block candidate found at height %d with %s share of difficulty %s by miner %s@%s',
            [job.height, shareType, hashDiff, miner.login, miner.ip]);
...
function handleMinerMethod(method, params, socket, portData, sendReply, pushMessage) {
    ...
    switch(method){
        case 'login':
            ...
            function registerConnection(params, socket, portData, pushMessage) {
                var minerId = uuidv4();
                var miner = new Miner(minerId, params.login, params.pass, socket, portData, pushMessage);
            ...
            if(portData.coin !== 'electroneum') {
                registerConnection(params, socket, portData, pushMessage);
            } else {
                (function(params, socket, portData, pushMessage) {
                ...
                })(params, socket, portData, pushMessage);
            }
            break;
        case 'getjob':
    ...
    async.each(config.poolServer.ports[coin], function(portData, cback){
        ...
        var onConnection = function(socket){
            ...
            socket.on('data', function(d){
                ...
                delete proxySockets[socket.remotePort];
                ...
                if(blackList.ips[ip]) {
                    dataBuffer = null;
                    log('warn', logSystem[coin], 'End socket from blacklisted IP %s', [ip]);
                    socket.destroy();
                    return;
                }
        ...
        tls.createServer(tlsOpts, onConnection).listen(portData.port - 1);
        net.createServer(null, onConnection).listen(portData.port + 1);
