...
var nonceReserve4Job = 15000000;
function BlockTemplate(template){
    this.difficulty = template.difficulty;
    this.height = template.height;
    this.buffer = new Buffer(template.blocktemplate_blob, 'hex');
    this.hashingBuffer = new Buffer(template.blockhashing_blob, 'hex');
    this.nonce = 0 - nonceReserve4Job;
}
BlockTemplate.prototype = {
    nextBlob: function(){
        this.nonce += nonceReserve4Job;
        if(this.nonce + nonceReserve4Job > maxInt) {
            log('error', logSystem, 
                'Failed to reserve nonce for next miner job at height %s, difficulty %s', 
                    [this.height, this.difficulty]);
        }
        this.hashingBuffer.writeUInt32BE(this.nonce, 39);
...
function processBlockTemplate(template, callback) {
    ...
    if(newTemplate) {
        async.each(connectedMiners, function(miner, callback) {
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch(method){
        ...
        case 'submit':
            ...
            blockTemplate.buffer = new Buffer(blockTemplate.buffer);
            ...
            if (shareAccepted !== 0) {
                switch(shareAccepted) {
                    case 1:
                        sendReply('Bad share. Check your miner configuration or contact pool administrator.');
                        break;
                    case 2:
                        sendReply('Rejected low difficulty share.');
                        break;
                    default:
                        log('error', logSystem, 'Unsupported share status %s', [shareAccepted]);
                }
                return;
            }
