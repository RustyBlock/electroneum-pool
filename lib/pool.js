...
function Miner(id, login, pass, ip, startingDiff, pushMessage, fixedDifficulty){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
Miner.prototype = {
    retarget: function(now){
        ...
        if(this.fixedDifficulty) return; // no retarget if we have fixed difficulty activated
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, fromCloud){
    ...
    actStats.saveMinerActivity(miner, job, shareDiff, dateNow);
}

function processShare(miner, job, blockTemplate, nonce, resultHash, fromCloud){
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    if (IsBannedIp(ip)){
        sendReply('your IP (' + ip + ') is banned');
        return;
    }
    ...
    switch(method){
        case 'login':
            ...
            if (addressParts.length > 1) {
                var afterDot = addressParts.slice(1).join('.');              
                diff = parseInt(afterDot);
                if(diff && diff.toString() !== afterDot) {
                    diff = Number.NaN;
                    log('warn', logSystem, 'Wrong number format specified as fixed difficulty request for %s', [params.login]);
                } else if(isNaN(diff)) {
                    log('warn', logSystem, 'Not a number specified as fixed difficulty request for %s', [params.login]);
                } else if(portData.fixedDifficulty !== true) { 
                    diff = Number.NaN;
                    log('info', logSystem, 'Declined fixed diffuculty of %s for miner %s', [afterDot, addressParts[0]]);
                } else {
                    log('info', logSystem, 'Miner %s is using %s for fixed difficulty', [addressParts[0], afterDot]);
                }
            }
            ...
            global.getWallet(params.login, function(result){
                ...
                miner = new Miner(minerId, params.login, params.pass, ip, diff || portData.difficulty, pushMessage, !isNaN(diff));
        ...
        case 'submit':
            ...
            var blockTemplate = currentBlockTemplate.height === job.height ? currentBlockTemplate : null/*validBlockTemplates.filter(function(t){
            ...
            var shareAccepted = processShare(miner, job, blockTemplate, params.nonce, params.result, portData.fixedDifficulty);
